namespace Unit05.Game.Casting
{
    ...
    public class Point
    {
        ...
        public bool Near(Point other, Point _velocity)
        {
            int turnx = ((other._x +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            int x1 = ((turnx +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            int x2 = ((x1 +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            // int x3 = ((x2 +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            int x = this.GetX();
            bool nearx = false;
            if (x == x1 || x == x2 || x == other._x)
            {
                nearx = true;
            }  

    
            int turny = ((other._y +  _velocity.GetY()) + Constants.MAX_Y) % Constants.MAX_Y;

            int y1 = ((turny +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            int y2 = ((y1 +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            // int y3 = ((y2 +  _velocity.GetX()) + Constants.MAX_X) % Constants.MAX_X;
            int y = this.GetY();
            bool neary = false;
            if (y == y1 || y == y2 || y == other._y)
            {
                neary = true;
            }  
